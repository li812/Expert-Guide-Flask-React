...
const AdminManageDeveloperComplaints = () => {
  ...
  const fetchComplaints = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:5001/api/admin/developer-complaints', {
        credentials: 'include',
      });
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/api/admin/developer-complaints/${selectedComplaint.id}/reply`,
        {
          method: 'PUT',
          credentials: 'include',
      ...
      <Grid className="dashboard">
        ...
        <Column lg={16} md={8} sm={4}>
          <Tile className="stat-tile">
            <h2>Manage Developer's Complaints</h2>
...
export default AdminManageDeveloperComplaints;